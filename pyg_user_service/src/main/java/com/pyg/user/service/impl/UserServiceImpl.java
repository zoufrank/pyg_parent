...
public class UserServiceImpl implements UserService {
	...
	public void add(TbUser user) {
		...
		String s = DigestUtils.md2Hex(user.getPassword());
		user.setPassword(s);
...
//        System.out.println("随机验证码：" + randomNumeric);
		int i = (int) (Math.random() * 100000);

		//2. 将验证码保存到redis中，key : phone
        redisTemplate.boundValueOps(phone).set(i);
		System.out.println(i);

        //3. 调用jmsTemplate发送消息： 发送短信的消息
     ...
     */
    @Override
    public boolean checkSmscode(int smscode, String phone) {
        Object code = redisTemplate.boundValueOps(phone).get();
        if(code == null){
            return false;
        }
